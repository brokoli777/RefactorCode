...
const modelMap = new Map([['1.5f', 'gemini-1.5-flash'], ['1.5p', 'gemini-1.5-pro']]);
...
program
  ...
  .action(async (inputPaths, options) => {
    ...
    if (!modelMap.has(selectedModel)) {
      stderr.write(chalk.red(`Invalid model specified. Choices: ${combinedModelsString}\n`));
      process.exit(1);
    }
...
async function refactor(filePath, outputFile, model, options) {
  try {
    ...
    if (options.stream) {
      ...
      const result = await geminiRefactor(text, model, filePath, outputFile, API_KEY);
